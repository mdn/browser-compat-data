...
Currently, the only allowed ranged version is `"≤37"` for `webview_android` and `"≤79"` for `edge`. There will be more ranged versions for other browsers in the future but ranged versions aren't generally allowed for every version string. Ranged versions should be used sparingly and only when it is impossible to find out the version number a feature initially shipped in.
