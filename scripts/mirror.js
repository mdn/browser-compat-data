...
const getMatchingBrowserVersion = (
  dest_browser,
  source_browser,
  source_browser_version,
) => {
  const range = source_browser_version.includes('≤');
  const source_browser_release =
    browsers[source_browser].releases[source_browser_version.replace('≤', '')];
  ...
  for (const r of releaseKeys) {
    ...
    if (
      ...
      return range ? `≤${r}` : r;
    } else if (release.engine == source_browser_release.engine) {
      if (
        ['beta', 'nightly'].includes(release.status) &&
        release.status == source_browser_release.status
      ) {
        return r;
      } else if (
