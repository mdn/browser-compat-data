...
function processData(filename, logger) {
  ...
  let expectedFeatureSorting = JSON.stringify(
    JSON.parse(actual),
    orderFeatures,
    2,
  );
  ...
  if (actual !== expected) {
    hasErrors = true;
    logger.error(
      chalk`{red Error on {bold line ${jsonDiff(actual, expected)}}}`,
    );
  }
  ...
  if (expected !== expectedBrowserSorting) {
    hasErrors = true;
    logger.error(
      chalk`{red Browser sorting error on {bold line ${jsonDiff(
        expected,
  ...
  if (actual !== expectedFeatureSorting) {
    hasErrors = true;
    logger.error(
      chalk`{red Feature sorting error on {bold line ${jsonDiff(
        expected,
  ...
  let constructorMatch = actual.match(
