...
function processData(filename) {
  ...
  processLink( // use https://bugzil.la/1000000 instead
    errors,
    actual,
    String.raw`https?://bugzilla\.mozilla\.org/show_bug\.cgi\?id=(\d+)`,
    match => {
      return {'issue': 'Use shortenable URL', 'expected': `https://bugzil.la/${match[1]}`};
    }
  );
  ...
  processLink( // use https://webkit.org/b/100000 instead
    errors,
    actual,
    String.raw`https?://bugs\.webkit\.org/show_bug\.cgi\?id=(\d+)`,
    match => {
      return {'issue': 'Use shortenable URL', 'expected': `https://webkit.org/b/${match[1]}`};
    }
  );

  processLink( // Bug links should use HTTPS and have "bug ###" as link text ("Bug ###" only at the begin of notes/sentences).
    errors,
    actual,
    ...
    match => {
      ...
      if (protocol !== 'https') {
        return {'issue': 'Use HTTPS for bug links', 'expected': `https://${domain}/${bugId}`, 'actualLink': url};
      }

      if (domain == 'bugzil.la') {
        if (/^bug $/.test(before)) {
          return {'issue': 'Move word "bug" into link text', 'expected': `<a href='...'>${before}${bugId}</a>`, 'actualLink': `${before}<a href='...'>${linkText}</a>`};
        } else if (linkText === `Bug ${bugId}`) {
          if (!/(\. |")$/.test(before)) {
            return {'issue': 'Use lowercase "bug" word within sentence', 'expected': `bug ${bugId}`, 'actualLink': `Bug ${bugId}`};
          }
        } else if (linkText !== `bug ${bugId}`) {
  ...
  processLink(
    ...
    match => {
      ...
      if (protocol !== 'https') {
        return {'issue': 'Use HTTPS MDN URL', 'expected': `https://developer.mozilla.org/${expectedPath}`};
      }

      if (subdomain) {
        return {'issue': 'Use correct MDN domain', 'expected': `https://developer.mozilla.org/${expectedPath}`};
      }
  ...
  processLink(
    ...
    match => {
      return {'issue': 'Use non-localized Microsoft Developer URL', 'expected': `https://developer.microsoft.com/${match[2]}`};
    }
  );
